...
namespace Nether.Analytics
{
    ...
    public class RunOncePerDaySchedule : IJobSchedule
    {
        ...
        /// <summary>
        /// This method is used to separate jobs that may have the same name but run on different
        /// intervals. E.g. job1 that runs at 4AM every day is *different* from the job that runs at 5AM every day
        /// </summary>
        /// <param name="shortJobName">"Short" name of the job</param>
        /// <returns>The detailed job name in the format {jobName}_daily_{hours}_{minutes}</returns>
        public string GetDetailedJobName(string shortJobName)
        {
            return $"{shortJobName}_daily_{_hours.ToString("00")}_{_minutes.ToString("00")}";
        }
