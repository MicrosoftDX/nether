...
namespace Nether.Data.Sql.Leaderboard
{
    public class ScoreContext : DbContext
    {
        ...
        private static string defaultSql = "select Score, GamerTag, CustomTag, row_number() over(order by Score desc) as Ranking from (select GamerTag, max(Score) as Score, max(CustomTag) as CustomTag from scores group by GamerTag) as T order by Score desc";
        private static string topSql = "select top {0} select Score, GamerTag, CustomTag, row_number() over(order by Score desc) as Ranking from (select GamerTag, max(Score) as Score, max(CustomTag) as CustomTag from scores group by GamerTag) as T order by Score desc";
        private static string aroundMeSql = "select top {0} from (select score, gamertag, customtag, rank() over(order by score desc) as ranking from scores s1 where score = (select max(score) from scores s2 where s1.gamertag = s2.gamertag)) as S where S.ranking >= {1} and S.gamertag != {2} union all select top {0} * from(select score, gamertag, customtag, rank() over(order by score desc) as ranking from scores s1 where score = (select max(score) from scores s2 where s1.gamertag = s2.gamertag)) as S where S.ranking < {1} ";
        private static string gamerRankSql = "select* from (select score, gamertag, customtag, rank() over(order by score desc) as ranking from scores s1 where score = (select max(score) from scores s2 where s1.gamertag = s2.gamertag)) as Ranks where Ranks.gamertag = {0}";
        ...
        {
            string sql = n == 0 ? defaultSql : String.Format(topSql, n);
        ...
        {
            string sql = String.Format(aroundMeSql, radius, rank, gamerTag);
        ...
        {
            string sql = String.Format(gamerRankSql, gamertag); 
