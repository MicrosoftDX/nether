...
namespace AnalyticsTestClient
{
    public class SchedulerJobMenu : ConsoleMenu
    {
        ...
        private async Task StartJobSchedulerAsync()
        {
            ...
            var everyHourOnTheHour = new EveryHourOnTheHourSchedule(stateProvider);
            ...
            await Task.Run(async () =>
            {
                while (true)
                {
                    ...
                    var job1Task = jobExecutor.RunAsSingletonAsync(job1, everyHourOnTheHour, async (d) =>
                    {
                    ...
                    var job2Task = jobExecutor.RunAsSingletonAsync(job2, everyHourOnTheHour, async (d) =>
                    {
                        var jobManager = new DataLakeAnalyticsJobManager(serviceClientCredentials,
                        Config.Root[Config.NAH_AZURE_SUBSCRIPTIONID], Config.Root[Config.NAH_AZURE_DLA_ACCOUNTNAME]);
                    ...
                    var job3Task = jobExecutor.RunAsSingletonAsync(job3, everyHourOnTheHour, async (d) =>
                    {
                        var jobManager = new DataLakeAnalyticsJobManager(serviceClientCredentials,
                        Config.Root[Config.NAH_AZURE_SUBSCRIPTIONID], Config.Root[Config.NAH_AZURE_DLA_ACCOUNTNAME]);
                    ...
                    await Task.Delay(TimeSpan.FromMinutes(5), s_cancellationToken);
                }
            }, s_cancellationToken);
        }
